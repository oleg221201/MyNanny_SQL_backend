...
router.post("/create/nanny", auth, async (req, res) => {
    try{
        ...
        const candidate = await Nanny.findOne({raw: true, where: {userId: req.user.id}})
...
router.get('/', auth, async (req, res) => {
    try{
        ...
        user = await Nanny.findOne({raw: true, where: {userId: req.user.id}})
        if (user) {
            return res.json({user: user, isCreated: true})
        }

        user = await Parent.findOne({raw: true, where: {userId: req.user.id}})
        if (user) {
            return res.json({user: user, isCreated: true})
        }
...
router.get('/:id', async (req, res) => {
    try{
        ...
        user = await Nanny.findOne({raw: true, where: {userId: req.params.id}})
        if (user) {
            return res.json({user: user})
        }

        user = await Parent.findOne({raw: true, where: {userId: req.params.id}})
        if (user) {
            return res.json({user: user})
        }
